...
async def _extract_stream(
        ...
        agent_name: str, event, response_state: Dict
) -> None:
    """ストリーミングから内容を抽出"""
...
async def invoke_agent(
        agent_name: str, query: str, mcp_client, create_agent,
        queue: Optional[asyncio.Queue]
) -> str:
    """サブエージェントを呼び出し"""
    response_state = {"text": ""}
    await send_event(
        queue, agent_name, f"サブエージェント「{agent_name}」が呼び出されました", "start"
    )
    ...
    try:
        ...
        await send_event(
            queue, agent_name, f"サブエージェント「{agent_name}」が調査を完了しました", "complete"
        )
